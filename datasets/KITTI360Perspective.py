...
from datasets.DataManagement import load_pc_file_fix_size, load_pc_files, loadCloudFromBinary
...
class KITTI360PerspectiveTriplet(Dataset):
    def __init__(self, config):
        ...
        if len(self.qImages) == 0 or len(self.dbImages) == 0:
            print_log("Exiting...", logger="KITTI360Perspective")
            print_log("There are no query/database images.", logger="KITTI360Perspective")
            print_log("Try more sequences", logger="KITTI360Perspective")
            sys.exit()
    ...
    def arange_as_seq(data:pd.DataFrame, path_img:str, path_pc:str, point_cloud_proxy:str=None):
        ...
        for seq_idx in data.index:
            ...
            seq_key = os.path.join(path_img, 'image_00', 'data_rect', '%010d' % img_num + '.png')
            if point_cloud_proxy == "points":
                seq_key_pc = os.path.join(path_pc, 'data_processed', '%010d' % img_num + '.bin')
            elif point_cloud_proxy == "bev_image": # TODO
                seq_key_pc = os.path.join(path_pc, 'bev', '%010d' % img_num + '.png')
            elif point_cloud_proxy == "range_image": # TODO
...
@DATASETS.register_module()
class KITTI360PerspectivePair(Dataset):
    def __init__(self, config):
        # config from dataset config file
        self.root_dir = config.data_path
        self.dataset_name = config.NAME
        ...
        # Note that the number of submap is same as the number of images
        if len(self.qImages) == 0 or len(self.dbImages) == 0:
            print_log("Exiting...", logger="KITTI360PerspectivePair")
            print_log("There are no query/database images.", logger="KITTI360PerspectivePair")
            print_log("Try more sequences", logger="KITTI360PerspectivePair")
            sys.exit()
        # cast to np.arrays for indexing during training
        self.qIdx = np.asarray(self.qIdx, dtype=object)
        self.pIdx = np.asarray(self.pIdx, dtype=object)
