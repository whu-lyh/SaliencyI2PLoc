...
class KITTI360PanoramaTriplet(Dataset):
    def __init__(self, config):
        ...
        # load data
        for sequence_idx in self.sequences:
            sequence_name = '2013_05_28_drive_%04d_sync' % int(sequence_idx)
            # print_log("Current sequence: " + sequence_name, logger="KITTI360Panorama")
            ...
            if self.mode == 'val' or self.mode == 'test':
                qData = pd.read_csv(os.path.join(self.root_dir, self.img_subdir, sequence_name, 'query_val.csv'), index_col=0)
                # load database data
                dbData = pd.read_csv(os.path.join(self.root_dir, self.img_subdir, sequence_name, 'database_val.csv'), index_col=0)
            else:
                qData = pd.read_csv(os.path.join(self.root_dir, self.img_subdir, sequence_name, 'query.csv'), index_col=0)
                # load database data
                dbData = pd.read_csv(os.path.join(self.root_dir, self.img_subdir, sequence_name, 'database.csv'), index_col=0)
            ...
            qSeqIdxs, qSeqKeys, qSeqKeys_pc = self.arange_as_seq(qData, 
                                                                os.path.join(self.root_dir, self.img_subdir, sequence_name), 
                                                                os.path.join(self.root_dir, self.submap_subdir, sequence_name),
            ...
            # print_log('self.qEndPosList: %i' % self.qEndPosList[-1], logger="KITTI360Panorama")
            # print_log('self.dbEndPosList: %i' % self.dbEndPosList[-1], logger="KITTI360Panorama")
            # utm coordinates
        ...
        if len(self.qImages) == 0 or len(self.dbImages) == 0:
            print_log("Exiting...", logger="KITTI360Panorama")
            print_log("There are no query/database images.", logger="KITTI360Panorama")
            print_log("Try more sequences", logger="KITTI360Panorama")
            sys.exit()
        ...
        # save the files for accelerate the dataset preprocessing
        if self.pre_save:
            np.save(os.path.join(self.root_dir, 'npys', 'kitti360_panorama_' + self.mode + '_qIdx.npy'), self.qIdx)
            np.save(os.path.join(self.root_dir, 'npys', 'kitti360_panorama_' + self.mode + '_pIdx.npy'), self.pIdx)
            np.save(os.path.join(self.root_dir, 'npys', 'kitti360_panorama_' + self.mode + '_nonNegIdx.npy'), self.nonNegIdx)
            np.save(os.path.join(self.root_dir, 'npys', 'kitti360_panorama_' + self.mode + '_qImages.npy'), self.qImages)
            np.save(os.path.join(self.root_dir, 'npys', 'kitti360_panorama_' + self.mode + '_dbImages.npy'), self.dbImages)
            np.save(os.path.join(self.root_dir, 'npys', 'kitti360_panorama_' + self.mode + '_qPcs.npy'), self.qPcs)
            np.save(os.path.join(self.root_dir, 'npys', 'kitti360_panorama_' + self.mode + '_dbPcs.npy'), self.dbPcs)
    ...
    def arange_as_seq(data:pd.DataFrame, path_img:str, path_pc:str, point_cloud_proxy:str=None):
        ...
        for seq_idx in data.index:
            ...
            seq_key = os.path.join(path_img, 'pano', 'data_rgb', '%010d' % img_num + '.png')
            if point_cloud_proxy == "points":
                seq_key_pc = os.path.join(path_pc, 'submaps', '%010d' % img_num + '.bin')
            elif point_cloud_proxy == "bev_image":
                seq_key_pc = os.path.join(path_pc, 'bev', '%010d' % img_num + '.png')
            elif point_cloud_proxy == "range_image":
                seq_key_pc = os.path.join(path_pc, 'range_image', '%010d' % img_num + '.png')
            else:
    ...
    def update_subcache(self, model=None, outputdim=None, margin=0.2):
        ...
        if self.current_subset >= len(self.subcache_indices):
            print_log('Reset epoch - FIX THIS LATER!', logger="KITTI360Panorama")
            self.current_subset = 0
        ...
        if model is None:
            ...
            return
        print_log("Online hard sample mining...", logger="KITTI360Panorama")
        ...
        with torch.no_grad():
            ...
            # release memory
            del nloader_pc
        print_log('Searching for hard negatives...', logger="KITTI360Panorama")
...
@DATASETS.register_module()
class KITTI360PanoramaPair(Dataset):
    def __init__(self, config):
        # config from dataset config file
        self.root_dir = config.data_path
        self.dataset_name = config.NAME
        ...
        # Note that the number of submap is same as the number of images
        if len(self.qImages) == 0 or len(self.dbImages) == 0:
            print_log("Exiting...", logger="KITTI360PanoramaPair")
            print_log("There are no query/database images.", logger="KITTI360PanoramaPair")
            print_log("Try more sequences", logger="KITTI360PanoramaPair")
            sys.exit()
        # cast to np.arrays for indexing during training
        self.qIdx = np.asarray(self.qIdx, dtype=object)
        self.pIdx = np.asarray(self.pIdx, dtype=object)
        ...
        # save the files for accelerate the dataset preprocessing
        if self.pre_save:
            path_dir = os.path.join(self.root_dir, 'npys_pair')
            if not os.path.exists(path_dir):
                os.makedirs(path_dir)
            np.save(os.path.join(path_dir, 'kitti360_panorama_' + self.mode + '_qIdx.npy'), self.qIdx)
            np.save(os.path.join(path_dir, 'kitti360_panorama_' + self.mode + '_pIdx.npy'), self.pIdx)
            np.save(os.path.join(path_dir, 'kitti360_panorama_' + self.mode + '_qImages.npy'), self.qImages)
            np.save(os.path.join(path_dir, 'kitti360_panorama_' + self.mode + '_dbImages.npy'), self.dbImages)
            np.save(os.path.join(path_dir, 'kitti360_panorama_' + self.mode + '_qPcs.npy'), self.qPcs)
            np.save(os.path.join(path_dir, 'kitti360_panorama_' + self.mode + '_dbPcs.npy'), self.dbPcs)
            if self.mode == 'train':
                np.save(os.path.join(path_dir, 'kitti360_panorama_' + self.mode + '_nonNegIdx.npy'), self.nonNegIdx)
                np.save(os.path.join(path_dir, 'kitti360_panorama_' + self.mode + '_pidxs.npy'), self.query_pidxs)
                np.save(os.path.join(path_dir, 'kitti360_panorama_' + self.mode + '_nidxs.npy'), self.query_nidxs)
    ...
    def arange_as_seq(data:pd.DataFrame, path_img:str, path_pc:str, point_cloud_proxy:str=None):
        ...
        for seq_idx in data.index:
            ...
            if point_cloud_proxy == "points":
                ...
                seq_key_pc = os.path.join(path_pc, 'bev', '%010d' % img_num + '.png')
                # seq_key_pc = os.path.join(path_pc, 'height_bev', '%010d' % img_num + '.png')
            elif point_cloud_proxy == "range_image":
                seq_key_pc = os.path.join(path_pc, 'range_image', '%010d' % img_num + '.png')
            else:
                raise NotImplementedError(f'Sorry, <{point_cloud_proxy}> is not implemented!')
    ...
    def _get_item_normal(self, idx):
        ...
        # query_pc = load_pc_files([self.qPcs[idx]], pts_num=self.points_num, augment=self.mode=='train')
        ...
        # positives_pcs = load_pc_files(pos_files_pc, pts_num=self.points_num, augment=self.mode=='train') # nPos N 3
    ...
    def _get_item_proxy_image(self, idx):
        ...
        # load images and pcs into list
        query = self.transform(Image.open(self.qImages[idx]).convert("RGB")).unsqueeze(0) # 1 C H W
        query_pc = self.transform_proxy(Image.open(self.qPcs[idx]).convert("RGB")).unsqueeze(0) # 1 C H W
        if self.contrast_mode: # return pair data
            return {'query_img': query, 'query_pc': query_pc}
        # query_pc = load_pc_files([self.qPcs[idx]], pts_num=self.points_num) # 1 N 3
        pos = np.where(self.query_is_pos_mask[idx, :])[0]
