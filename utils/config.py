...
def get_config(args, logger=None):
    if args.resume:
        # config information is copied once after loading
...
def save_experiment_config(args, config, logger=None):
    ...
    config_path = os.path.join(args.experiment_path, 'config-whole.yaml')
    with open(config_path, 'w') as f:
        yaml.dump(config,f, indent=2)
    print_log(f'Save whole configurations to {config_path}', logger=logger)