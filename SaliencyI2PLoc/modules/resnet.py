...
class ResNet(nn.Module):
    '''
        ResNet architecture which could be adjusted based on block list value
        FPN structure is also enclosed
    '''
...
class ResNetEncoder(ResNet):
    ...
    def from_config(cls, config, from_pretrained=False):
        ...
        if arch_type == "resnet18":
            encoder = resnet18(in_channels=3, pretrained=from_pretrained, progress=True)
        elif arch_type == "resnet34":
            encoder = resnet34(in_channels=3, pretrained=from_pretrained, progress=True)
        elif arch_type == "resnet50":
            encoder = resnet50(in_channels=3, pretrained=from_pretrained, progress=True)
        elif arch_type == "resnet101":
            encoder = resnet101(in_channels=3, pretrained=from_pretrained, progress=True)
        elif arch_type == "resnet152":
            encoder = resnet152(in_channels=3, pretrained=from_pretrained, progress=True)
        elif arch_type == "resnext50_32x4d":
            encoder = resnext50_32x4d(in_channels=3, pretrained=from_pretrained, progress=True)
        elif arch_type == "resnext101_32x8d":
            encoder = resnext101_32x8d(in_channels=3, pretrained=from_pretrained, progress=True)
        elif arch_type == "wide_resnet50_2":
            encoder = wide_resnet50_2(in_channels=3, pretrained=from_pretrained, progress=True)
        elif arch_type == "wide_resnet101_2":
            encoder = wide_resnet101_2(in_channels=3, pretrained=from_pretrained, progress=True)
        else:
            encoder = resnet18(in_channels=3, pretrained=from_pretrained, progress=True)      
        return encoder
...
class attention_pc2img(nn.Module):
    def __init__(self, in_channel, output_channel):
        ...
        '''
        self.conv=nn.Sequential(nn.Conv2d(in_channel,in_channel,1),
                                nn.BatchNorm2d(in_channel),nn.ReLU(),
                                nn.Conv2d(in_channel,in_channel,1),
                                nn.BatchNorm2d(in_channel),nn.ReLU(),
                                nn.Conv2d(in_channel,output_channel,1),
                                nn.BatchNorm2d(output_channel),nn.ReLU())
        '''
        self.conv=nn.Sequential(ResidualConv(in_channel, in_channel),
                                ResidualConv(in_channel, in_channel),
                                nn.Conv2d(in_channel, output_channel, 1),
    ...
    def forward(self, pc_global_feature, img_local_feature, pc_local_feature):
        ...
        feature = torch.cat([img_local_feature, 
                        pc_global_feature.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, H, W)],
                        dim=1)
...
class ImageUpSample(nn.Module):
    def __init__(self, in_channel, output_channel):
        ...
        self.conv = nn.Sequential(ResidualConv(in_channel, output_channel),
                                  ResidualConv(output_channel, output_channel))
